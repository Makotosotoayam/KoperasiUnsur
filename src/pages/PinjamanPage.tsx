...
export function PinjamanPage() {
    ...
    const [items, setItems] = useState<PinjamanRow[]>([]);
    ...
    const [editing, setEditing] = useState<PinjamanRow | null>(null);
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            // Only rows where pokok or jasa is nonzero — a "pinjaman" record
            // for this view means an actual loan deduction happened that month.
            ...
            let query = supabase
                ...
                .select('id, id_anggota, bulan, tahun, pokok, jasa, cicilan_ke, anggota!inner(id, no_kku, nama, id_unit, unit_kerja(id, nama_unit))', { count: 'exact' })
                .eq('tahun', filterTahun)
                .or('pokok.gt.0,jasa.gt.0')
                .order('bulan')
            ...
            const rows = (data as any) as PinjamanRow[];
    ...
    const openEdit = (item: PinjamanRow) => {
        setEditing(item);
        setForm({
            id_anggota: item.id_anggota.toString(),
            bulan: item.bulan,
            tahun: item.tahun,
            pokok: String(item.pokok),
            jasa: String(item.jasa),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const cicilan_ke = form.cicilan_ke || null;

            if (editing) {
                const { error: err } = await supabase
                    .from('transaksi_potongan')
                    .update({ pokok, jasa, cicilan_ke })
                    .eq('id', editing.id);
                if (err) throw err;
            } else {
                ...
                if (existing) {
                    ...
                    const { error: err } = await supabase
                        .from('transaksi_potongan')
                        .insert({ id_anggota, bulan: form.bulan, tahun: form.tahun, pokok, jasa, cicilan_ke });
                    if (err) throw err;
                }
            }
    ...
    const handleDelete = async (item: PinjamanRow) => {
        if (!confirm(`Kosongkan data pinjaman ${item.anggota?.nama} untuk ${MONTHS[item.bulan - 1]} ${item.tahun}? (Data simpanan/pengeluaran bulan ini tidak akan terhapus)`)) return;
        try {
            // Only clear pokok/jasa/cicilan_ke — this row may still hold simpanan/pengeluaran data.
            const { error: err } = await supabase
                .from('transaksi_potongan')
                .update({ pokok: 0, jasa: 0, cicilan_ke: null })
                .eq('id', item.id);
            if (err) throw err;
            load();
        } catch (err) {
            console.error(err);
            alert('Gagal menghapus data pinjaman');
        }
    };

    return (
        <div className="p-6 max-w-7xl mx-auto">
            <PageHeader title="Pinjaman" description="Angsuran pokok & jasa pinjaman anggota (per bulan)" actionLabel="Tambah Pinjaman" onAction={openCreate}>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : items.length === 0 ? (
                    <EmptyState message="Belum ada data pinjaman untuk periode ini" />
                ) : (
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100">
                                    ...
                                    <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase">Cicilan Ke</th>
                                    <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase text-right">Pokok</th>
                                    <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase text-right">Jasa</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {items.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50/50">
                                        ...
                                        <td className="py-3 px-6 text-sm text-gray-600">{item.cicilan_ke ?? '-'}</td>
                                        <td className="py-3 px-6 text-sm text-gray-700 text-right">{formatIDR(Number(item.pokok))}</td>
                                        <td className="py-3 px-6 text-sm text-gray-700 text-right">{formatIDR(Number(item.jasa))}</td>
                                        <td className="py-3 px-6 text-sm font-medium text-gray-900 text-right">{formatIDR(Number(item.pokok) + Number(item.jasa))}</td>
            ...
            <Modal open={modalOpen} title={editing ? 'Edit Pinjaman' : 'Tambah Pinjaman'} onClose={() => setModalOpen(false)}>
                <form onSubmit={handleSubmit}>
                    ...
                    <FormField label="Cicilan Ke (mis. 3/10)">
                        <input className={inputClass} placeholder="3/10" value={form.cicilan_ke} onChange={(e) => setForm({ ...form, cicilan_ke: e.target.value })} />
                    </FormField>
                    <div className="grid grid-cols-2 gap-3">
                        <FormField label="Pokok (Rp)" required>
                            <input type="number" min="0" className={inputClass} required value={form.pokok} onChange={(e) => setForm({ ...form, pokok: e.target.value })} />
                        </FormField>
                        <FormField label="Jasa (Rp)">
                            <input type="number" min="0" className={inputClass} value={form.jasa} onChange={(e) => setForm({ ...form, jasa: e.target.value })} />
                        </FormField>
                    </div>
