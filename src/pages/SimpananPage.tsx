...
export function SimpananPage() {
    ...
    const [items, setItems] = useState<SimpananRow[]>([]);
    ...
    const [editing, setEditing] = useState<SimpananRow | null>(null);
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            let query = supabase
                ...
                .select('id, id_anggota, bulan, tahun, simwa, sukarela, anggota!inner(id, no_kku, nama, id_unit, unit_kerja(id, nama_unit))', { count: 'exact' })
            ...
            const rows = (data as any) as SimpananRow[];
    ...
    const openEdit = (item: SimpananRow) => {
        setEditing(item);
        setForm({
            id_anggota: item.id_anggota.toString(),
            bulan: item.bulan,
            tahun: item.tahun,
            simwa: String(item.simwa),
            sukarela: String(item.sukarela),
        });
        setModalOpen(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const simwa = Number(form.simwa) || 0;
            const sukarela = Number(form.sukarela) || 0;

            if (editing) {
                const { error: err } = await supabase
                    .from('transaksi_potongan')
                    .update({ simwa, sukarela })
                    .eq('id', editing.id);
                if (err) throw err;
            } else {
                // Check if a row already exists for this member+month+year
                // (row may already hold pinjaman/pengeluaran data — update it, don't duplicate).
                ...
                if (existing) {
                    ...
                    const { error: err } = await supabase
                        .from('transaksi_potongan')
                        .insert({ id_anggota, bulan: form.bulan, tahun: form.tahun, simwa, sukarela });
                    if (err) throw err;
                }
            }
    ...
    const handleDelete = async (item: SimpananRow) => {
        if (!confirm(`Kosongkan data simpanan ${item.anggota?.nama} untuk ${MONTHS[item.bulan - 1]} ${item.tahun}? (Data pinjaman/pengeluaran bulan ini tidak akan terhapus)`)) return;
        try {
            // Only zero out simwa/sukarela — this row may still hold pinjaman/pengeluaran data.
            const { error: err } = await supabase
                .from('transaksi_potongan')
                .update({ simwa: 0, sukarela: 0 })
                .eq('id', item.id);
            if (err) throw err;
            load();
        } catch (err) {
            console.error(err);
            alert('Gagal menghapus data simpanan');
        }
    };

    return (
        <div className="p-6 max-w-7xl mx-auto">
            <PageHeader title="Simpanan" description="Simpanan wajib (SIMWA) & sukarela anggota koperasi" actionLabel="Tambah Simpanan" onAction={openCreate}>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                {loading ? <LoadingState /> : error ? <ErrorState message={error} /> : items.length === 0 ? (
                    <EmptyState message="Belum ada data simpanan untuk periode ini" />
                ) : (
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100">
                                    ...
                                    <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase text-right">SIMWA</th>
                                    <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase text-right">Sukarela</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {items.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50/50">
                                        ...
                                        <td className="py-3 px-6 text-sm text-gray-700 text-right">{formatIDR(Number(item.simwa))}</td>
                                        <td className="py-3 px-6 text-sm text-gray-700 text-right">{formatIDR(Number(item.sukarela))}</td>
                                        <td className="py-3 px-6 text-sm font-medium text-gray-900 text-right">{formatIDR(Number(item.simwa) + Number(item.sukarela))}</td>
            ...
            <Modal open={modalOpen} title={editing ? 'Edit Simpanan' : 'Tambah Simpanan'} onClose={() => setModalOpen(false)}>
                <form onSubmit={handleSubmit}>
                    ...
                    <FormField label="SIMWA (Rp)" required>
                        <input type="number" min="0" className={inputClass} required value={form.simwa} onChange={(e) => setForm({ ...form, simwa: e.target.value })} />
                    </FormField>
                    <FormField label="Sukarela (Rp)">
                        <input type="number" min="0" className={inputClass} value={form.sukarela} onChange={(e) => setForm({ ...form, sukarela: e.target.value })} />
                    </FormField>
